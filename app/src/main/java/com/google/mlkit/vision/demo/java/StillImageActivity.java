...
public final class StillImageActivity extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    findViewById(R.id.addToDatabaseButton).setOnClickListener(view -> {
      EditText et = findViewById(R.id.nameEditText);
      String etValue = et.getText().toString();
      String formattedEtValue = etValue.toLowerCase();
      DateViewModel model = new ViewModelProvider(this).get(DateViewModel.class);
      String date = model.getDate();


      RequestQueue queue = Volley.newRequestQueue(this);
      String url = "https://us-central1-expire-list-302608.cloudfunctions.net/add_object";

      JSONObject postData = new JSONObject();
      try {
        postData.put("name", formattedEtValue);
        postData.put("expiry_date", date);
      ...
      Toast.makeText(
              ...
              "You have added " + etValue + " to the database. With expiry date " + date,
              Toast.LENGTH_LONG)
              .show();
    });
  }
  ...
  private void tryReloadAndDetectInImage() {
    ...
    try {
      ...
      if (imageProcessor != null) {
        ...
        Log.e(TAG, "Null , please check adb logs for imageProcessor creation error");
      }
    } catch (IOException e) {
